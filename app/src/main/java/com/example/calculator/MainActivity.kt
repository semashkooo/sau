...
//Операторы
        ...
        tvFactorial?.setOnClickListener {
            try {
                var numberForFactorial: Int = tvExpression.text.toString().toInt()

                var n = numberForFactorial
                var result = 1
                for (i in 1..n) {
                    result *= i
                }
                tvResult.text = result.toBigInteger().toString()
            }catch (e:Exception){
                Log.d("Ошибка", "Ошибка: ${e.message}")
            }
        }
        tvKoren?.setOnClickListener {
            try {
                var numberForKoren: Double = tvExpression.text.toString().toDouble()
                tvResult.text = sqrt(numberForKoren).toString()


            }catch (e: Exception){
                Log.d("Exception", "massege : " + e.message)
            }
        }
        //Очистка
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        outState.run {
            ...
            putString("Key1", tvResult.text.toString())
        }
    ...
    override fun onRestoreInstanceState(savedInstanceState: Bundle) {
        ...
        tvResult.text = savedInstanceState.getString("Key1")
    }
}
